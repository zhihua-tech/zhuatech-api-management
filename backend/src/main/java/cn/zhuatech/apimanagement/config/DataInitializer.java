...
package cn.zhuatech.apimanagement.config;
import cn.zhuatech.apimanagement.domain.DomainCatalog;
import cn.zhuatech.apimanagement.model.*;
import cn.zhuatech.apimanagement.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "APIM-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="APIM-DEMO-"+String.format("%03d",sequence);
